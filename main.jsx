...
function openMailtoSafe(mailto){
  try{
...
function openEmailReminderFixed(inv,c){
  try{
    requirePdfForCobranza(inv);
    if(!c?.email){
    ...
    openMailtoSafe(emailReminder(inv,c));
  }catch(err){
    alert("No se pudo abrir el correo: "+(err?.message||err));
  }
}
